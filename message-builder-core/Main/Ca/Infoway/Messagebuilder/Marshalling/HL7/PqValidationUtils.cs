...
namespace Ca.Infoway.Messagebuilder.Marshalling.HL7
{
	public class PqValidationUtils
	{
		...
		static PqValidationUtils()
		{
			...
			maximum_fraction_digits_exceptions[Hl7BaseVersion.MR2007_V02R01 + "_PQ.DRUG"] = 2;
			maximum_fraction_digits_exceptions[Hl7BaseVersion.MR2007_V02R01 + "_PQ.LAB"] = 2;
		...
		public virtual Type GetUnitTypeByHl7Type(StandardDataType dataType, string propertyPath, Hl7Errors errors)
		{
			return GetUnitTypeByHl7Type(dataType == null ? null : dataType.Type, null, propertyPath, errors);
		}

		public virtual Type GetUnitTypeByHl7Type(string typeAsString, XmlElement element, string propertyPath, Hl7Errors errors)
		{
			StandardDataType type = StandardDataType.GetByTypeName(typeAsString);
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								{
									CreateError(System.String.Format("Type \"{0}\" is not a valid PQ type", typeAsString), element, propertyPath, errors);
		...
		public virtual BigDecimal ValidateValue(string value, VersionNumber version, string type, XmlElement element, string propertyPath
			, Hl7Errors errors)
		{
			...
			else
			{
				if (NumberUtil.IsNumber(value))
				{
					...
					{
						CreateError(System.String.Format(errorMessage, version.GetBaseVersion(), type, maxFractionDigits, "decimal"), element, propertyPath
							, errors);
					}
					if (StringUtils.Length(integerPart) > maxIntDigits)
					{
						CreateError(System.String.Format(errorMessage, version.GetBaseVersion(), type, maxIntDigits, "integer"), element, propertyPath
							, errors);
					}
					if (!StringUtils.IsNumeric(integerPart) || !StringUtils.IsNumeric(decimalPart))
					{
						alreadyWarnedAboutValue = true;
						CreateError(System.String.Format("value \"{0}\" must contain digits only", value), element, propertyPath, errors);
					}
				}
				try
				{
				...
				catch (FormatException)
				{
					...
					{
						CreateError(System.String.Format("value \"{0}\" is not a valid decimal value", value), element, propertyPath, errors);
					}
				}
			}
			return result;
			...
			, XmlElement element, string propertyPath, Hl7Errors errors)
		{
			...
			if (StringUtils.IsNotBlank(unitsAsString))
			{
				units = (Ca.Infoway.Messagebuilder.Domainvalue.UnitsOfMeasureCaseSensitive)CodeResolverRegistry.Lookup(this.GetUnitTypeByHl7Type
					(type, element, propertyPath, errors), unitsAsString);
				if (units == null)
				{
					CreateError(System.String.Format("Unit \"{0}\" is not valid for type {1}", unitsAsString, type), element, propertyPath, errors
						);
				}
			}
			return units;
