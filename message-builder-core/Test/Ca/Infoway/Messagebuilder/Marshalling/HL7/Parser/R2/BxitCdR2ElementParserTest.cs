...
{
	[TestFixture]
	public class BxitCdR2ElementParserTest : MarshallingTestCase
	{
		...
		public virtual void TestParseEmptyNode()
		{
			...
			BXIT<CodedTypeR2<MockCharacters>> cd = (BXIT<CodedTypeR2<MockCharacters>>)this.parser.Parse(ParseContextImpl.Create("BXIT<CD>"
				, typeof(MockCharacters), SpecificationVersion.V02R02, null, null, Ca.Infoway.Messagebuilder.Xml.ConformanceLevel.MANDATORY
				, null, null, false), node, this.xmlResult);
			Assert.IsTrue(this.xmlResult.IsValid());
		...
		public virtual void TestParseInvalidNullFavorAttributeValue()
		{
			...
			this.parser.Parse(ParseContextImpl.Create("BXIT<CD>", typeof(MockCharacters), SpecificationVersion.V02R02, null, null, Ca.Infoway.Messagebuilder.Xml.ConformanceLevel
		...
		{
			XmlNode node = CreateNode("<something code=\"BARNEY\" codeSystem=\"1.2.3.4.5\" codeSystemName=\"aCsName\" codeSystemVersion=\"aCsVersion\" displayName=\"aDisplayName\" value=\"1.2\" operator=\"P\" qty=\"6677\">"
			...
			BXIT<CodedTypeR2<MockCharacters>> cd = (BXIT<CodedTypeR2<MockCharacters>>)this.parser.Parse(ParseContextImpl.Create("BXIT<CD>"
				...
				, null, null, false), node, this.xmlResult);
			Assert.IsFalse(this.xmlResult.IsValid());
			Assert.AreEqual(3, this.xmlResult.GetHl7Errors().Count, "error message count");
			...
			Assert.AreEqual("normal", cd.Value.Qualifier[0].Value.GetCodeValue());
			Assert.AreEqual("ACT", cd.Value.Qualifier[1].Value.GetCodeValue());
			Assert.IsNull(cd.Value.Operator);
			Assert.AreEqual(6677, cd.Value.Qty);
		}
	}
}
