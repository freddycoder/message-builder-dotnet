...
namespace Ca.Infoway.Messagebuilder.Model.Pcs_mr2007_v02_r02.Common.Merged {
     ...
     * changes to all of them must be either accepted or rejected 
     * as a single action. I.e. One trigger event = 1 unit of 
     * work.</p> MCAI_MT700210CA.Subject2: f:refers to <p>On 
     * creation interactions where the state transition is 
     * null-&gt;active, contextConductionInd must be true, 
     * otherwise it must be false.</p> <p>Indicates the item(s) 
     * being acted upon by this trigger event.</p><p>Will be 
     * mandatory in wrappers with payload messages, otherwise not 
     * present.</p><p>If true, associations in the parent act are 
     ...
     * dataEnterer association is carried according to the 
     * dataEnterer.contextControlCode (propogate to the payload and 
     * add to any data enterer(s) already present in the 
     * payload).</p> <p>Indicates the item(s) being acted upon by 
     * this trigger event.</p><p>Will be mandatory in wrappers with 
    ...
    public class RefersTo_1<ACT> : MessagePartBean {
         ...
         * payload).</p> <p>Indicates whether or not objects of the 
