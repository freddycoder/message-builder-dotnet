/**
 * Copyright 2012 Canada Health Infoway, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *        http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * Author:        $LastChangedBy: tmcgrady $
 * Last modified: $LastChangedDate: 2012-01-18 21:07:18 -0500 (Wed, 18 Jan 2012) $
 * Revision:      $LastChangedRevision: 3957 $
 */

/* This class was auto-generated by the message builder generator tools. */
namespace Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Repc_mt230003ca {
    using Ca.Infoway.Messagebuilder.Annotation;
    using Ca.Infoway.Messagebuilder.Datatype;
    using Ca.Infoway.Messagebuilder.Datatype.Impl;
    using Ca.Infoway.Messagebuilder.Datatype.Lang;
    using Ca.Infoway.Messagebuilder.Model;
    using Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged;
    using System.Collections.Generic;


    [Hl7PartTypeMappingAttribute(new string[] {"REPC_MT230003CA.Section"})]
    public class Section : MessagePartBean {

        private ED<EncapsulatedData> documentOverviewContent;
        private Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.IDocumentContent_3 component1DocumentContent;
        private IList<Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.DocumentSections> component2SubSection;
        private IList<Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.Reference> component3Reference;

        public Section() {
            this.documentOverviewContent = new EDImpl<EncapsulatedData>();
            this.component2SubSection = new List<Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.DocumentSections>();
            this.component3Reference = new List<Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.Reference>();
        }
        /**
         * <summary>M: Document Overview Content</summary>
         * 
         * <remarks><p>Provides the primary rendered textual content of 
         * the document.</p><p>E.g. The cover letter for a referral, 
         * the overview portion of a report, etc.</p> <p>Provides the 
         * primary rendered textual content of the document.</p><p>E.g. 
         * The cover letter for a referral, the overview portion of a 
         * report, etc.</p> <p>Represents the principle content of the 
         * document and therefore is mandatory.</p> <p>There is no 
         * guidance provided within the standard on how report content 
         * should be organized or formatted. Guidance may be provided 
         * by professional colleges, specialty groups or others.</p></remarks>
         */
        [Hl7XmlMappingAttribute(new string[] {"text"})]
        public EncapsulatedData DocumentOverviewContent {
            get { return this.documentOverviewContent.Value; }
            set { this.documentOverviewContent.Value = value; }
        }

        [Hl7XmlMappingAttribute(new string[] {"component1/documentContent"})]
        public Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.IDocumentContent_3 Component1DocumentContent {
            get { return this.component1DocumentContent; }
            set { this.component1DocumentContent = value; }
        }

        public Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.ClinicalObservationDocument Component1DocumentContentAsObservation {
            get { return this.component1DocumentContent is Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.ClinicalObservationDocument ? (Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.ClinicalObservationDocument) this.component1DocumentContent : (Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.ClinicalObservationDocument) null; }
        }
        public bool HasComponent1DocumentContentAsObservation() {
            return (this.component1DocumentContent is Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.ClinicalObservationDocument);
        }

        [Hl7XmlMappingAttribute(new string[] {"component2/subSection"})]
        public IList<Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.DocumentSections> Component2SubSection {
            get { return this.component2SubSection; }
        }

        [Hl7XmlMappingAttribute(new string[] {"component3/reference"})]
        public IList<Ca.Infoway.Messagebuilder.Model.Pcs_mr2009_r02_04_02.Iehr.Merged.Reference> Component3Reference {
            get { return this.component3Reference; }
        }

    }

}